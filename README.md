...
以網頁 App（可加到手機主畫面）查詢台灣衛福部食藥署《農藥殘留容許量標準》附表一。可**依藥劑**、**依作物**，或**交叉**查出容許量（ppm）。資料內建，使用時不必連食藥署。不使用 LINE。
...
離開家裡網路時，這個本機網址會失效。請用 **GitHub Pages**（推送到 `main` 後自動發佈）：

`https://你的帳號.github.io/pesticide-mrl/`
